...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    console.error(`[API Error] ${status} ${requestUrl}`, error.response?.data || error.message);
    ...
    if (status === 401) {
      ...
      const isAlreadyOnLogin = window.location.pathname === '/' || window.location.pathname === '/signup';

      if (!isAuthCall && !isAlreadyOnLogin) {
        console.warn('[API Auth] Session expired – clearing token and redirecting to login');

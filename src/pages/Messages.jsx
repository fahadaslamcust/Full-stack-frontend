...
export default function Messages() {
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row h-full bg-[#f8f8f8] overflow-hidden relative w-full">
      ...
      {/* ====== CHAT SCREEN ====== */}
      <section
        className={`flex-1 bg-[#fbfbfb] flex flex-col min-w-0 h-full ${!isChatListOpen ? "flex" : "hidden md:flex"}`}
      >
        {!selectedChat ? (
              ...
              {chatLoading ? (
                ...
                messages.map((msg, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`max-w-[75%] sm:max-w-md px-4 py-2.5 rounded-2xl text-xs sm:text-sm break-words ${isOwn ? "bg-blue-500 text-white rounded-tr-none shadow-xs" : "bg-white text-gray-800 rounded-tl-none border border-gray-100 shadow-xs"}`}
                      >
                        {msg.content || msg.text}
                        <span
                          className={`block text-[10px] mt-1 ${isOwn ? "text-blue-200" : "text-gray-400"}`}
                        >
                          {msg.createdAt
      ...
      {isCalling && (
        <div className="fixed inset-0 bg-[#f8f9fa] z-50 flex flex-col items-center justify-center animate-fade-in">
          ...
          <p className="text-xs text-gray-500 mt-1 animate-pulse font-medium">
            Ongoing Call...
          </p>
          <p className="text-xs text-gray-400 mt-0.5 font-mono bg-gray-100 px-2 py-0.5 rounded-md">
            00:00
          </p>
          <div className="absolute bottom-16 flex items-center gap-4">
            ...
            >
              <PhoneOff
                size={18}
                className="text-white group-hover:rotate-135 transition-transform duration-300"
              />
            </button>
          </div>
        </div>
